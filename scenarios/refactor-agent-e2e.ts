...
const SCENARIO_ID = "refactor-agent-e2e";
...
const fixtureDir = join(harnessDir, "fixtures", "refactor-agent-e2e");
...
async function assertHappyPathCleanup(repoRoot: string): Promise<string | undefined> {
  ...
  for (const [idx, file] of getResponseFileNames().entries()) {
    const stem = file.replace(/\.json$/, "");
    const marker = `__mock_${idx}_${stem}.txt`;
 ...
 *     summary.json                ← parseable; agent === "refactor"
 *     events.jsonl                ← non-empty
 *     step-01-agent.{system,user,response}   ← refactor step
 *     step-02-summary.{system,user,response} ← summary writer
 *
 * The deterministic check between the refactor step and the summary writer also
 * emits a `step-01-det-iter-01.log`, but it's not part of the contract this
 * harness pins down — the bug-fix and feat-small counterparts don't assert it
 * either.
 *
...
function assertArtifacts(): string | undefined {
  ...
  const requiredFiles = [
    ...
    "step-02-summary.system.md",
    "step-02-summary.user.md",
    "step-02-summary.response.txt",
  ];
  ...
  // summary.json must parse and have agent === "refactor"
  ...
  if (
    ...
    summary === null ||
    (summary as Record<string, unknown>).agent !== "refactor"
  ) {
    return `summary.json has wrong shape or wrong agent value (expected agent === "refactor")`;
  }
...
async function run(): Promise<ScenarioResult> {
  ...
  try {
    ...
    // Install the refactor skill into the sandbox so the agent's
    // `.withSkill("munchkins:refactor")` resolves against the sandboxed repo root,
    ...
    const skillDestDir = join(sandbox.path, ".claude", "skills", "munchkins-refactor");
    mkdirSync(skillDestDir, { recursive: true });
    copyFileSync(skillSrc, join(skillDestDir, "SKILL.md"));

    const userMessagePath = join(sandbox.path, "refactor.md");
    ...
    const agent = registry.get("refactor");
    if (!agent) {
      return failResult(
        "setup",
        'registry.get("refactor") returned undefined — bundle import did not register the agent',
      );
    }
