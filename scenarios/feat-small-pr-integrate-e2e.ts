#!/usr/bin/env bun
/**
 * feat-small agent with `--integrate=pr` end-to-end.
 *
 * Reuses the `feat-small-agent-e2e` fixtures verbatim — same mocked pipeline,
...
const SCENARIO_ID = "feat-small-pr-integrate-e2e";
...
// Reuse feat-small-agent-e2e fixtures — the PR variant doesn't change agent
...
async function run(): Promise<ScenarioResult> {
  ...
  try {
    ...
    // Install the feat-small skill into the sandbox.
    const skillSrc = join(
      repoRoot,
      "packages",
      "serrano-munchkins",
    ...
    process.env.__MUNCHKINS_OPT_userMessage = join(sandbox.path, "feature.md");
    ...
    if (!agent) {
      return failResult("setup", 'registry.get("feat-small") returned undefined');
    }
