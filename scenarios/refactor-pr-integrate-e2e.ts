#!/usr/bin/env bun
/**
 * Refactor agent with `--integrate=pr` end-to-end.
 *
 * Owns its own fixture directory (does not share with the refactor Lights out
 * slice that is being authored in parallel). The fake `gh` shim plus a
 * bare-repo `origin` cover the push + PR-create surface so no real GitHub
 * call is made and no real `claude` invocation occurs.
 *
 * Assertions:
 *   - agent.run() succeeded
...
const SCENARIO_ID = "refactor-pr-integrate-e2e";
...
// Dedicated fixture dir for the refactor PR slice — does NOT share with the
// parallel refactor Lights out slice.
const fixtureDir = join(harnessDir, "fixtures", "refactor-pr-integrate-e2e");
...
async function run(): Promise<ScenarioResult> {
  ...
  try {
    ...
    // Install the refactor skill into the sandbox.
    const skillSrc = join(
      repoRoot,
      "packages",
      "serrano-munchkins",
    ...
    process.env.__MUNCHKINS_OPT_userMessage = join(sandbox.path, "refactor.md");
    ...
    if (!agent) {
      return failResult("setup", 'registry.get("refactor") returned undefined');
    }
