#!/usr/bin/env bun
/**
 * Director agent with `--integrate=pr` end-to-end.
 *
 * Drives one director tick whose mocked triage step emits an upstream-idle
 * short-circuit so dispatch exits 0 without invoking a child munchkin. The
 * scenario then asserts the director's *own* integration boundary — push to
 * a bare-repo `origin` + `gh pr create` via the PATH-prepended fake `gh`
 * shim — exactly like the feat-small / bug-fix Foreman scenarios.
 *
 * Closes PURPOSE.md Success #1 for the director agent (Foreman coverage).
 * The Lights out (multi-dispatch) coverage lives in
 * scenarios/director-multi-dispatch-e2e.ts; this scenario is intentionally
 * narrower so dispatch never spawns a real child run.
 *
 * Assertions:
 *   - agent.run() succeeded
 *   - zero real `claude` / `cmux` spawn attempts (audit guard)
 *   - `gh pr create` invoked exactly once with `--base main`, `--title`, `--body`
 *   - the PR body contains the `**Goal:**` marker emitted by the summary-writer fixture
 *   - local main did NOT advance (PR strategy never ff-merges)
 *   - the bare remote received an `agent/*` branch whose tip subject starts
 *     with `docs(changelog):` (the summary-writer phase's terminal commit)
 *   - no `.worktrees/` entries, no `agent/*` local branches survive teardown
 *   - the run-log's summary.json reports `agent === "director"`
 */
...
const SCENARIO_ID = "director-pr-integrate-e2e";
...
function findDirectorRunLogDir(): string | undefined {
  if (!existsSync(artifactDir)) return undefined;
  for (const name of readdirSync(artifactDir)) {
    const abs = join(artifactDir, name);
    if (!statSync(abs).isDirectory()) continue;
    const summaryPath = join(abs, "summary.json");
    if (existsSync(summaryPath)) return abs;
  }
  return undefined;
}
...
async function run(): Promise<ScenarioResult> {
  ...
  try {
    ...
    // Match the director's declared default explicitly; the cron daemon would
    // pass this through, and the dispatch script's idle short-circuit needs
    // the agent steps to run (which they do regardless of the message value).
    process.env.__MUNCHKINS_OPT_userMessage = "tick";
    ...
    const agent = registry.get("director");
    if (!agent) {
    ...
    // Audit guard: zero real claude / cmux invocations.
    ...
    // Run-log summary.json identifies the director.
    const runLogDir = findDirectorRunLogDir();
    if (!runLogDir) {
      return failResult(
        "artifact",
        `no run-log subdir with summary.json found under ${artifactDir}`,
      );
    }
    const summary = JSON.parse(readFileSync(join(runLogDir, "summary.json"), "utf-8")) as {
      agent?: string;
    };
    if (summary.agent !== "director") {
      return failResult(
        "artifact",
        `summary.json agent === ${JSON.stringify(summary.agent)}; expected "director"`,
      );
    }
