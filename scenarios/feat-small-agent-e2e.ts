...
const SCENARIO_ID = "feat-small-agent-e2e";
...
const fixtureDir = join(harnessDir, "fixtures", "feat-small-agent-e2e");
 ...
 *     summary.json                ← parseable; agent === "feat-small"
 ...
 *
 * The deterministic check between test-writer and summary writer also emits a
 * `step-04-det-iter-01.log`, but it's not part of the contract this harness
 * pins down — the bug-fix counterpart doesn't assert it either.
 *
...
function assertArtifacts(): string | undefined {
  ...
  const requiredFiles = [
    ...
    "step-03-agent.system.md",
    "step-03-agent.user.md",
    "step-03-agent.response.txt",
    "step-04-summary.system.md",
    "step-04-summary.user.md",
    "step-04-summary.response.txt",
  ];
  ...
  // summary.json must parse and have agent === "feat-small"
  ...
  if (
    ...
    summary === null ||
    (summary as Record<string, unknown>).agent !== "feat-small"
  ) {
    return `summary.json has wrong shape or wrong agent value (expected agent === "feat-small")`;
  }
...
async function run(): Promise<ScenarioResult> {
  ...
  try {
    ...
    // Install the feat-small skill into the sandbox so the agent's
    // `.withSkill("munchkins:feat-small")` resolves against the sandboxed repo root,
    ...
    const skillDestDir = join(sandbox.path, ".claude", "skills", "munchkins-feat-small");
    mkdirSync(skillDestDir, { recursive: true });
    copyFileSync(skillSrc, join(skillDestDir, "SKILL.md"));

    const userMessagePath = join(sandbox.path, "feature.md");
    ...
    const agent = registry.get("feat-small");
    if (!agent) {
      return failResult(
        "setup",
        'registry.get("feat-small") returned undefined — bundle import did not register the agent',
      );
    }
